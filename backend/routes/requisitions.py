...
async def update_requisition(req_id: str, data: dict, current_user: User = Depends(get_current_user)):
    ...
    if not has_perm(current_user, "manage_requisitions", "approve_purchase"):
...
async def fulfill_requisition(req_id: str, payload: dict = {}, current_user: User = Depends(get_current_user)):
    ...
    for item in req["items"]:
        # Polymorphic subject — ingredient or product. The override map is
        # keyed by whichever id was used at creation time.
        if item.get("ingredient_id"):
            subject_field = "ingredient_id"
            sid = item["ingredient_id"]
        else:
            subject_field = "product_id"
            sid = item.get("product_id")
        qty = int(overrides.get(sid, item["quantity_requested"]))
        if qty <= 0:
        ...
        await db.stock.update_one(
            {subject_field: sid, "outlet_id": outlet_id, "store": "main"},
            {"$inc": {"quantity": -qty}},
        )

        # Add to destination store (upsert)
        existing = await db.stock.find_one(
            {subject_field: sid, "outlet_id": outlet_id, "store": from_store}
        )
        if existing:
            await db.stock.update_one(
                {subject_field: sid, "outlet_id": outlet_id, "store": from_store},
