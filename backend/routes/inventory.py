...
# ─── Bulk requisition CSV — transfer many rows from one store to another ──────
# Each CSV row maps to a single Main → target-store transfer (equivalent to
# calling /api/stock/transfer for that ingredient). Skips rows where the
# ingredient doesn't exist (we don't auto-create here — the source store
# can't have stock of an ingredient that's never been received). Skips rows
# where the source store lacks enough quantity. Every successful row writes
# a stock_movements log entry so the Transfer Record page reflects the
# bulk operation as individual transfers, keeping the audit trail honest.
#
# CSV format: name, quantity[, notes]
# Form params: outlet_id, to_store, from_store (default 'main'), file

@router.post("/stock/requisition-csv")
async def requisition_stock_csv(
    outlet_id: str = Form(...),
    to_store: str = Form(...),
    from_store: str = Form("main"),
    file: UploadFile = File(...),
    current_user: User = Depends(get_current_user),
):
    _require_manager(current_user)
    if to_store == from_store:
        raise HTTPException(400, "from_store and to_store must differ")
    try:
        contents = (await file.read()).decode("utf-8-sig")
    except Exception:
        raise HTTPException(400, "Could not read file — ensure it is a valid UTF-8 CSV.")
    ...
    reader = csv.DictReader(io.StringIO(contents))
    results: dict = {"transferred": 0, "skipped": 0, "errors": []}

    from_outlet = _normalize_outlet_for_store(from_store, outlet_id)
    to_outlet   = _normalize_outlet_for_store(to_store,   outlet_id)
    ...
    for row in reader:
        ...
        try:
            ...
            continue
        if qty <= 0:
            results["errors"].append(f'"{name}" has zero quantity — skipped')
            results["skipped"] += 1
            continue
        ...
        if not ingredient:
            results["errors"].append(f'Ingredient not found: "{name}" — receive it into Main Store first')
            results["skipped"] += 1
            continue

        source = await db.stock.find_one(
            {"ingredient_id": ingredient["id"], "outlet_id": from_outlet, "store": from_store},
            {"_id": 0},
        )
        if not source or float(source.get("quantity", 0)) < qty:
            available = float(source.get("quantity", 0)) if source else 0
            results["errors"].append(f'"{name}": only {available} available in {from_store}')
            results["skipped"] += 1
            continue
        ...
        dest = await db.stock.find_one(
            {"ingredient_id": ingredient["id"], "outlet_id": to_outlet, "store": to_store},
            {"_id": 0, "id": 1, "min_quantity": 1},
        )
        if dest:
            await db.stock.update_one(
                {"ingredient_id": ingredient["id"], "outlet_id": to_outlet, "store": to_store},
        ...
        await db.stock_movements.insert_one({
            "id": str(_uuid.uuid4()),
            "ingredient_id": ingredient["id"],
            "subject_name": ingredient["name"],
            "from_store": from_store,
            "to_store": to_store,
            "outlet_id": outlet_id,
            "quantity": qty,
            "type": "transfer",
            "notes": (row.get("notes") or f"Bulk requisition from {from_store} to {to_store}").strip(),
