...
async def create_group(group_data: CategoryCreate, current_user: User = Depends(get_current_user)):
    if not has_perm(current_user, "manage_products", "manage_groups", "manage_categories", "create_product", "edit_product", "delete_product", "import_products"):
...
async def delete_product(product_id: str, current_user: User = Depends(get_current_user)):
    if not has_perm(current_user, "delete_product", "manage_products"):
        raise HTTPException(status_code=403, detail="Not authorized")
