...
from routes.reports import _dedupe_orders

router = APIRouter(prefix="/api")


def require_admin(user: User):
    """Allow admin / manager always, or any user holding an accounting permission."""
    if not has_perm(
        user,
        "view_accounts", "view_accounts_dashboard",
        "manage_expenses", "manage_deposits", "manage_transfers",
...
async def get_ledger(
    ...
    if not entry_type or entry_type == "sales":
        orders = await db.orders.find(order_q, {"_id": 0, "id": 1, "created_at": 1, "total": 1, "order_number": 1, "payment_method": 1, "table_id": 1, "table_number": 1, "bar_tab_id": 1, "status": 1, "customer_name": 1, "created_by": 1, "created_by_name": 1}).to_list(2000)
        orders = _dedupe_orders(orders)
        for o in orders:
            entries.append({
