...
async def get_image(image_id: str):
    ...
    # Images are public assets. Send permissive CORS headers directly on the
    # response so cross-origin <img crossOrigin> loads and fetch() calls
    # from the Vercel frontend succeed regardless of CORS_ORIGINS env config
    # (the global CORSMiddleware refuses to send '*' when credentials=true,
    # which leaves image responses without any allow-origin header).
    return Response(
        content=content,
...
async def get_image_preflight(image_id: str):
    # Browsers may issue a preflight for <img crossOrigin="anonymous">; reply
    # quickly with the same permissive headers.
    return Response(
        status_code=204,
        headers={
            "Access-Control-Allow-Origin": "*",
