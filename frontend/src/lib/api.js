...
export const api = {
  ...
  ping: () => fetch(`${BASE_URL}/api/ping`, { method: "GET", cache: "no-store" })
    .then((r) => r.ok)
    .catch(() => false),
  ...
  recallOrder: (id) =>
    request(`/api/orders/${id}/recall`, { method: "POST" }),
  splitOrderItems: (id, payload) =>
    request(`/api/orders/${id}/split-items`, { method: "POST", body: JSON.stringify(payload) }),
  ...
  // Inventory / Stock
  // bust=true appends a one-shot cache-busting param so the service worker
  // can't serve a stale list — used immediately after a mutation when we
  // need to confirm the new value landed.
  getStock: (outlet_id, store, bust = false) => {
    const params = new URLSearchParams();
  ...
  absorbMergedTable: (tableId, mergeTableId) =>
    request(`/api/tables/${tableId}/absorb-merged`, { method: "POST", body: JSON.stringify({ merge_table_id: mergeTableId }) }),
  moveOrderToTable: (fromTableId, toTableId) =>
    request(`/api/tables/${fromTableId}/move-order`, { method: "POST", body: JSON.stringify({ to_table_id: toTableId }) }),
  ...
  // Wipe Image Library and unlink products that referenced library images
  clearImageLibrary: () => request("/api/admin/clear-image-library", { method: "POST" }),

  // Wipe every order created by a specific user
  clearUserOrders: ({ user_id, user_name }) =>
    request("/api/admin/clear-user-orders", { method: "POST", body: JSON.stringify({ user_id, user_name }) }),
