...
import { Plus, Trash2, ToggleLeft, ToggleRight, X, Shield, ChevronDown, ChevronRight, Users, Pencil, Camera, Eraser } from "lucide-react";
...
const PERMISSION_MODULES = [
  {
    ...
    permissions: [
      ...
      { id: "manage_units",        label: "Manage Units" },
      { id: "manage_recipes",      label: "Manage Recipes" },
      { id: "print_labels",        label: "Print Labels" },
      { id: "manage_image_library", label: "Manage Image Library" },
    ],
  },
  {
  ...
  },
  {
    ...
    permissions: [
      { id: "view_stores",           label: "View Stores (Main Store)" },
      { id: "create_store",          label: "Create Store" },
      { id: "edit_store",            label: "Edit Store" },
      { id: "delete_store",          label: "Delete Store" },
      { id: "view_transfer_record",  label: "View Transfer Record" },
    ],
  },
  {
    id: "orders", label: "Orders",
    permissions: [
      { id: "use_pos",                 label: "Access POS / Sales Screen" },
      ...
      { id: "recall_order",            label: "Recall Completed Order" },
      { id: "edit_completed_order",    label: "Edit Completed Order (after recall)" },
      { id: "split_bill",              label: "Split Bill at Checkout" },
      { id: "reprint_kitchen_ticket",  label: "Reprint Kitchen / Bar Ticket" },
    ],
  },
  {
    ...
    permissions: [
      ...
      { id: "manage_tables",   label: "Manage Tables (create/edit/delete)" },
      { id: "assign_table",    label: "Assign Table to Order" },
      { id: "transfer_tables", label: "Transfer Table (reassign waiter)" },
      { id: "move_order",      label: "Move Order to Another Table" },
      { id: "merge_tables",    label: "Merge / Unmerge Tables" },
      { id: "release_tables",  label: "Release / Clear Table" },
      { id: "print_bill",      label: "Print Bill" },
    ],
  },
  {
    id: "floors", label: "Floor Management",
    permissions: [
      { id: "view_floors",   label: "View Floors" },
      { id: "create_floor",  label: "Create Floor" },
      { id: "edit_floor",    label: "Edit Floor" },
      { id: "delete_floor",  label: "Delete Floor" },
      { id: "manage_floors", label: "Manage Floors (full access incl. tables on each floor)" },
    ],
  },
  {
    id: "bar_tabs", label: "Bar Tabs",
    permissions: [
      { id: "view_bar_tabs",   label: "View Bar Tabs" },
      { id: "manage_bar_tabs", label: "Manage Bar Tabs" },
      { id: "close_bar_tab",   label: "Close Bar Tab" },
      { id: "delete_bar_tab",  label: "Delete Bar Tab" },
    ],
  },
  {
    ...
    permissions: [
      ...
      { id: "create_reservation",  label: "Create Reservation" },
      { id: "edit_reservation",    label: "Edit Reservation" },
      { id: "cancel_reservation",  label: "Cancel Reservation" },
      { id: "delete_reservation",  label: "Delete Reservation" },
    ],
  },
  {
  ...
  },
  {
    ...
    permissions: [
      ...
      { id: "view_accounts_dashboard",  label: "View Accounts Dashboard" },
      { id: "view_profit_loss",         label: "View Profit & Loss" },
      { id: "view_balance_sheet",       label: "View Balance Sheet" },
      { id: "manage_cash_flow",         label: "View Cash Flow" },
      { id: "view_general_ledger",      label: "View General Ledger" },
      { id: "view_invoice_tracking",    label: "View Invoice Tracking" },
      { id: "view_bank_reconciliation", label: "View Bank Reconciliation" },
      { id: "view_budgets",             label: "View Budgets & Forecast" },
      { id: "view_tax_vat",             label: "View Tax & VAT" },
      { id: "manage_expenses",          label: "Manage Expenses & Categories" },
      { id: "manage_deposits",          label: "Manage Deposits & Categories" },
      { id: "manage_transfers",         label: "Manage Transfers" },
      { id: "manage_payroll",           label: "Manage Payroll" },
      { id: "manage_currencies",        label: "Manage Currencies & Rates" },
    ],
  },
  {
  ...
  },
  {
    ...
    permissions: [
      ...
      { id: "manage_terminals",  label: "Manage Terminals" },
    ],
  },
  {
    ...
    permissions: [
      ...
      { id: "manage_service_mode",     label: "Manage Service Mode" },
      { id: "manage_printer_settings", label: "Manage Printer Settings" },
    ],
  },
];
...
const ADD_USER_PERMISSIONS = {
  ...
  pos: [
    ...
    { id: "recall_order", label: "Recall Completed Orders" },
    { id: "edit_completed_order", label: "Edit Completed Orders" },
    { id: "split_bill", label: "Split Bill at Checkout" },
    { id: "reprint_kitchen_ticket", label: "Reprint Kitchen / Bar Tickets" },
    { id: "manage_tables", label: "Manage Tables" },
    { id: "transfer_tables", label: "Transfer Tables (waiter)" },
    { id: "move_order", label: "Move Order to Another Table" },
    { id: "merge_tables", label: "Merge / Unmerge Tables" },
...
function UserList() {
  ...
  const handleClearOrders = async (u) => {
    if (!window.confirm(
      `Wipe every order created by "${u.name}"?\n\n` +
      `Held / sent-to-kitchen / pending / completed orders are all deleted. ` +
      `Tables and bar tabs that were holding those orders get released, and ` +
      `linked customers have their total_orders / total_spent rolled back.\n\n` +
      `This cannot be undone.`
    )) return;
    try {
      const res = await api.clearUserOrders({ user_id: u.id, user_name: u.name });
      alert(
        `${res.orders_deleted} order(s) deleted.\n` +
        `Tables released: ${res.tables_released}\n` +
        `Bar tabs closed: ${res.bar_tabs_closed}\n` +
        `Customers adjusted: ${res.customers_adjusted}`
      );
    } catch (err) { alert(err.message); }
  };
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                      <button onClick={() => openEdit(u)} title="Edit user" className="text-blue-500 hover:text-blue-700 transition-colors">
                        <Pencil size={15} />
                      </button>
                      <button onClick={() => handleClearOrders(u)} title="Wipe this user's order history" className="text-orange-500 hover:text-orange-700 transition-colors">
                        <Eraser size={15} />
                      </button>
                      <button onClick={() => handleDelete(u)} title="Delete user" className="text-red-400 hover:text-red-600 transition-colors">
                        <Trash2 size={15} />
                      </button>
                    </div>
                  </div>
...
function CreateUserTypeModal({ onClose, onSave, editType = null }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editType) {
        // Backend returns users_updated — the count of staff whose
        // permissions were refreshed by this edit. Show it so the admin
        // can confirm the change actually propagated (was the original
        // complaint: 'I updated the type but the users still had the
        // old permissions').
        const res = await api.updateUserType(editType.id, { name: name.trim(), permissions: selectedPerms });
        const n = res?.users_updated ?? 0;
        onSave(n > 0 ? `Type updated. ${n} user${n === 1 ? "" : "s"} re-synced to the new permissions.` : null);
      } else {
...
function UserTypes() {
  ...
  const [flash, setFlash] = useState("");  // success banner after a save / delete
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.deleteUserType(id); load("User type deleted."); }
  ...
  return (
    <div>
      ...
      {flash && (
        <div className="fixed bottom-6 right-6 z-50 bg-emerald-600 text-white px-4 py-3 rounded-xl shadow-xl text-sm font-semibold max-w-sm">
          {flash}
        </div>
      )}
    </div>
