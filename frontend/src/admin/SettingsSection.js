...
function CompanySettings() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      // Remap the form fields to the backend BusinessSettings field names so
      // they actually persist (backend ignores unknown keys silently).
      const payload = {
      ...
      if (logoFile) {
        try {
          ...
          payload.logo_url = url; // legacy field, kept for back-compat
        } catch (uploadErr) {
...
function TaxSettings() {
  ...
  const [shiftHours, setShiftHours] = useState({
    morning_start: "06:00", afternoon_start: "12:00", evening_start: "17:00", night_start: "22:00",
  });
  ...
  useEffect(() => {
    if (settings) {
      ...
      setShiftHours({
        morning_start:   settings.morning_start   || "06:00",
        afternoon_start: settings.afternoon_start || "12:00",
        evening_start:   settings.evening_start   || "17:00",
        night_start:     settings.night_start     || "22:00",
      });
    }
  }, [settings]);

  const saveShiftHours = async () => {
    if (savingShift) return;
    setSavingShift(true);
    try {
      await api.updateSettings(shiftHours);
      await refreshSettings();
    } catch (err) { alert(err.message); }
  ...
  return (
    <div>
      ...
      {/* Shift Hours */}
      <div className="bg-white dark:bg-gray-800 rounded-2xl border-2 border-gray-300 dark:border-gray-600 shadow-sm p-6 mb-6">
        <h3 className="font-bold text-gray-900 dark:text-white mb-1">Shift Hours</h3>
        <p className="text-xs text-gray-400 mb-4">
          Set when each shift begins. Each shift runs from its start time until the next one begins.
          Night wraps past midnight back to Morning. The Shift Report groups orders into these four buckets.
        </p>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { key: "morning_start",   label: "🌅  Morning starts",   next: "afternoon_start" },
            { key: "afternoon_start", label: "☀️  Afternoon starts", next: "evening_start" },
            { key: "evening_start",   label: "🌆  Evening starts",   next: "night_start" },
            { key: "night_start",     label: "🌙  Night starts",     next: "morning_start" },
          ].map(({ key, label, next }) => (
            <div key={key}>
              <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1.5">
                {label}
              </label>
              <input type="time" value={shiftHours[key]}
                onChange={(e) => setShiftHours({ ...shiftHours, [key]: e.target.value })}
              ...
              <p className="text-[10px] text-gray-400 mt-1">
                → ends at <span className="font-semibold text-gray-600 dark:text-gray-300">{shiftHours[next]}</span>
              </p>
            </div>
          ))}
        </div>
        <div className="flex justify-end mt-4">
          <button onClick={saveShiftHours} disabled={savingShift}
